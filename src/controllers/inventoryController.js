...
exports.getInventory = async (req, res) => {
    try {
        ...
        const [totalResult] = await pool.query('SELECT SUM(current_quantity) as total FROM inventory WHERE current_quantity > 0 AND deleted_at IS NULL');
...
exports.getInventoryItem = async (req, res) => {
    try {
        // جلب بيانات العينة سواء محذوفة أو لا
...
exports.getEditForm = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM inventory WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
        if (rows.length === 0) {
            return res.status(404).render('error', { message: 'العينة غير موجودة' });
        }
...
exports.toggleReject = async (req, res) => {
    try {
        const [currentItem] = await pool.query('SELECT is_rejected FROM inventory WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
...
// نقل عينات إلى سلة المحذوفات
exports.trashItems = async (req, res) => {
    try {
        const { ids } = req.body;
        ...
        if (!ids || !Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'يرجى اختيار عينة واحدة على الأقل'
            });
        }

        // تحديث السجلات المحددة
        const [result] = await pool.query(
            'UPDATE inventory SET deleted_at = NOW() WHERE id IN (?)',
            [ids]
        );

        if (result.affectedRows > 0) {
            req.flash('success_msg', `تم نقل ${result.affectedRows} عينة إلى سلة المحذوفات بنجاح`);
            res.json({
                success: true,
                message: `تم نقل ${result.affectedRows} عينة إلى سلة المحذوفات بنجاح`,
                affectedRows: result.affectedRows
            });
        } else {
            res.status(404).json({
                success: false,
                message: 'لم يتم العثور على العينات المحددة'
            });
        }
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: 'حدث خطأ أثناء نقل العينات إلى سلة المحذوفات'
        });
    }
};

// نقل عينة واحدة إلى سلة المحذوفات
exports.trashItem = async (req, res) => {
    try {
        const { id } = req.params;
        
        // التحقق من وجود العينة
        const [item] = await pool.query(
            'SELECT id, sample_number FROM inventory WHERE id = ? AND deleted_at IS NULL',
            [id]
        );
        ...
        if (item.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'لم يتم العثور على العينة أو أنها محذوفة بالفعل'
            });
        }

        // نقل العينة إلى سلة المحذوفات
        const [result] = await pool.query(
            'UPDATE inventory SET deleted_at = NOW() WHERE id = ?',
            [id]
        );
        ...
        if (result.affectedRows > 0) {
            res.json({
                success: true,
                message: `تم نقل العينة ${item[0].sample_number} إلى سلة المحذوفات بنجاح`
            });
        } else {
            res.status(500).json({
                success: false,
                message: 'حدث خطأ أثناء نقل العينة إلى سلة المحذوفات'
            });
        }
    } catch (error) {
...
exports.getDeletedItems = async (req, res) => {
    try {
        const { date, supplier, sample_number } = req.query;
        let sql = `
            SELECT *, COALESCE(is_rejected, 0) as is_rejected FROM inventory 
        ...
        const [deletedItems] = await pool.query(sql, params);

        res.render('inventory/deleted', {
            deletedItems,
            filters: req.query,
            user: req.session.user
        });
        ...
        res.status(500).render('error', {
            message: 'حدث خطأ أثناء جلب بيانات سلة المحذوفات',
            error: process.env.NODE_ENV === 'development' ? error : {}
        });
    }
};
...
exports.restoreItem = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            'UPDATE inventory SET deleted_at = NULL WHERE id = ? AND deleted_at IS NOT NULL',
            [id]
        );
        ...
        if (result.affectedRows > 0) {
            res.json({
                success: true,
                message: 'تم استعادة العينة بنجاح'
            });
        } else {
            res.status(404).json({
                success: false,
                message: 'لم يتم العثور على العينة أو أنها غير محذوفة'
            });
        }
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: 'حدث خطأ أثناء استعادة العينة'
        });
    }
};

// استعادة عينات متعددة من سلة المحذوفات
exports.restoreMultipleItems = async (req, res) => {
    try {
        const { ids } = req.body;
        ...
        const [result] = await pool.query(
            'UPDATE inventory SET deleted_at = NULL WHERE id IN (?) AND deleted_at IS NOT NULL',
            [ids]
        );

        if (result.affectedRows > 0) {
            req.flash('success_msg', `تم استعادة ${result.affectedRows} عينة بنجاح`);
            res.json({
                success: true,
                message: `تم استعادة ${result.affectedRows} عينة بنجاح`,
                affectedRows: result.affectedRows
            });
        } else {
            res.status(404).json({
                success: false,
                message: 'لم يتم العثور على العينات المحددة أو أنها غير محذوفة'
            });
        }
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: 'حدث خطأ أثناء استعادة العينات'
        });
    }
};

// حذف جميع السجلات المحذوفة نهائياً
exports.emptyTrash = async (req, res) => {
    try {
        const [result] = await pool.query('DELETE FROM inventory WHERE deleted_at IS NOT NULL');
        req.flash('success_msg', `تم تفريغ سلة المحذوفات (${result.affectedRows} سجل)`);
        res.json({ success: true, message: `تم تفريغ سلة المحذوفات (${result.affectedRows} سجل)` });
    } catch (error) {
