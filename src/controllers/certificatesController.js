...
exports.delete = async (req, res) => {
  try {
    const [result] = await req.db.execute(
      'UPDATE certificates SET deleted_at = NOW() WHERE id = ?',
      [req.params.id]
    );
    ...
    res.json({ success: true, message: 'تم نقل الشهادة إلى سلة المحذوفات بنجاح' });
  } catch (error) {
    console.error(error);
...
// نقل محددات إلى سلة المحذوفات (soft delete)
exports.trashMultiple = async (req, res) => {
  try {
    ...
    if (!Array.isArray(ids) || !ids.length) {
      return res.status(400).json({ 
        success: false, 
        message: 'اختر شهادة واحدة على الأقل' 
      });
    }

    // إنشاء placeholders للـ IN clause
    const placeholders = ids.map(() => '?').join(',');
    const query = `UPDATE certificates SET deleted_at = NOW() WHERE id IN (${placeholders})`;
    
    await req.db.execute(query, ids);

    res.json({ 
      success: true, 
      message: `تم نقل ${ids.length} شهادة إلى سلة المحذوفات` 
    });
  } catch (error) {
    console.error('Error trashing certificates:', error);
    res.status(500).json({ 
      success: false, 
      message: 'حدث خطأ أثناء نقل الشهادات إلى سلة المحذوفات' 
    });
  }
};
...
exports.getDeletedCertificates = async (req, res) => {
  try {
    ...
    req.flash('error', 'حدث خطأ أثناء تحميل سلة المحذوفات');
    res.redirect('/certificates');
  }
};
...
exports.restoreMultiple = async (req, res) => {
  try {
    ...
    if (!Array.isArray(ids) || !ids.length) {
      return res.status(400).json({ 
        success: false, 
        message: 'اختر شهادة واحدة على الأقل للاستعادة' 
      });
    }

    // إنشاء placeholders للـ IN clause
    const placeholders = ids.map(() => '?').join(',');
    const query = `UPDATE certificates SET deleted_at = NULL WHERE id IN (${placeholders})`;
    
    await req.db.execute(query, ids);

    res.json({ 
      success: true, 
      message: `تم استعادة ${ids.length} شهادة` 
    });
  } catch (error) {
    console.error('Error restoring certificates:', error);
    res.status(500).json({ 
      success: false, 
      message: 'حدث خطأ أثناء استعادة الشهادات' 
    });
  }
};

// تفريغ سلة المحذوفات (حذف دائم)
exports.emptyTrash = async (req, res) => {
  try {
    await req.db.execute(
      'DELETE FROM certificates WHERE deleted_at IS NOT NULL'
    );

    res.json({ 
      success: true, 
      message: 'تم تفريغ سلة المحذوفات بنجاح' 
    });
  } catch (error) {
    console.error('Error emptying trash:', error);
    res.status(500).json({ 
      success: false, 
      message: 'حدث خطأ أثناء تفريغ سلة المحذوفات' 
    });
  }
};
...
exports.deleteMultiple = async (req, res) => {
  try {
    ...
    if (!Array.isArray(ids) || !ids.length) {
      return res.status(400).json({ 
        success: false, 
        message: 'اختر شهادة واحدة على الأقل للحذف' 
      });
    }

    // إنشاء placeholders للـ IN clause
    const placeholders = ids.map(() => '?').join(',');
    const query = `DELETE FROM certificates WHERE id IN (${placeholders})`;
    
    await req.db.execute(query, ids);

    res.json({ 
      success: true, 
      message: `تم حذف ${ids.length} شهادة نهائياً` 
    });
  } catch (error) {
    console.error('Error deleting certificates:', error);
    res.status(500).json({ 
      success: false, 
      message: 'حدث خطأ أثناء حذف الشهادات' 
    });
  }
};
...
exports.restore = async (req, res) => {
  try {
    const [result] = await req.db.execute(
      'UPDATE certificates SET deleted_at = NULL WHERE id = ?',
      [req.params.id]
    );
    ...
    res.status(500).json({ 
      success: false, 
      message: 'حدث خطأ أثناء استعادة الشهادة' 
    });
  }
}; 